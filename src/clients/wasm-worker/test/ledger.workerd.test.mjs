...
// These tests run inside `workerd` itself (via `@cloudflare/vitest-pool-workers`, using
// wrangler.toml), not a Node mock of it — they prove `tb_wasm.wasm` actually loads and runs the
// production TigerBeetle state machine/LSM under Cloudflare's own Workers runtime, driven
// through the `TigerBeetleLedger` Durable Object exactly as a deployed Worker would use it.
// See ledger.celld.test.mjs for the equivalent suite against celld (using wrangler.jsonc) —
// same TigerBeetleLedger code, unmodified, run against both runtimes.
